...
function DropdownPanel({ items, open }) {
    return (
        <div className={`absolute left-0 top-full z-50 pt-2 transition-[opacity,transform] duration-150 ease-out ${open ? "visible translate-y-0 opacity-100" : "invisible -translate-y-1 opacity-0"}`}>
            <div className="w-[220px] rounded-sm border border-border bg-white shadow-xl">
                <div className="grid max-h-[320px] gap-0 py-0.5 overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                    {items.map((it) => (
                        <Link
                            key={it.href}
                            href={it.href}
...
function DroprightPanel({ sections, open }) {
                ...
                {sections.map((sec) => {
                    ...
                    return (
                        <div
                            ...
                            onMouseEnter={() => setOpenSec(sec.heading)}
                            onMouseLeave={() => setOpenSec(null)}
                        >
                            <div className="flex items-center justify-between gap-2 px-4 py-3 text-sm font-medium text-foreground/80 transition-colors hover:bg-black/5 hover:text-foreground">
                                <Link
                                    href={
                                        sec.viewAll?.href ?? sec.items[0]?.href
                                    }
                            ...
                            <div className={`absolute left-full top-0 z-10 -ml-[1px] w-[240px] rounded-sm rounded-l-none border border-border bg-white p-3 shadow-xl transition-[opacity,transform] duration-150 ease-out ${openSec === sec.heading ? "visible translate-x-0 opacity-100" : "invisible -translate-x-1 opacity-0"}`}>
...
function MegaDropdownPanel({ sections, open }) {
    // alias untuk kompatibilitas — sekarang dropright untuk destinasi/budaya
    return <DroprightPanel sections={sections} open={open} />;
}
                    ...
                    {menu.map((m) =>
                        m.direct ? (
                            ...
                            <div
                                ...
                                onMouseEnter={() => setDeskOpen(m.key)}
